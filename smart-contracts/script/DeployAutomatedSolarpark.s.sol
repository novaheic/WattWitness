// SPDX-License-Identifier: MIT
pragma solidity 0.8.19;

import {Script, console2} from "forge-std/Script.sol";
import {AutomatedSolarpark} from "../src/AutomatedSolarpark.sol";

/**
 * @title DeployAutomatedSolarpark
 * @notice This script deploys the AutomatedSolarpark contract for WattWitness solar park data logging with Chainlink Automation
 * @dev Run with: forge script script/DeployAutomatedSolarpark.s.sol:DeployAutomatedSolarpark --rpc-url <RPC_URL> --private-key <PRIVATE_KEY> --broadcast
 */
contract DeployAutomatedSolarpark is Script {
    // Network configurations
    struct NetworkConfig {
        address router;
        bytes32 donId;
        uint64 subscriptionId;
        uint32 gasLimit;
    }

    // Simple Fetch Readings CBOR Request (generated from corrected simple-fetch-readings.js)
    // This fetches pending readings from WattWitness API at /api/v1/readings/pending
    bytes constant SIMPLE_FETCH_REQUEST = hex"";

    // Network configurations - update these based on the network you're deploying to
    function getNetworkConfig() internal view returns (NetworkConfig memory) {
        // Avalanche Fuji testnet configuration
        return NetworkConfig({
            router: 0xA9d587a00A31A52Ed70D6026794a8FC5E2F5dCb0, // Fuji Functions Router
            donId: 0x66756e2d6176616c616e6368652d66756a692d31000000000000000000000000, // fun-avalanche-fuji-1
            subscriptionId: uint64(vm.envUint("CHAINLINK_FUNCTIONS_SUBSCRIPTION_ID")), // Get from .env
            gasLimit: 300000
        });
    }

    function run() external {
        uint256 deployerPrivateKey = vm.envUint("DEPLOYER_PRIVATE_KEY");
        
        NetworkConfig memory config = getNetworkConfig();
        
        console2.log("Deploying AutomatedSolarpark contract...");
        console2.log("Router address:", config.router);
        console2.log("DON ID:", vm.toString(abi.encodePacked(config.donId)));
        console2.log("Subscription ID:", config.subscriptionId);
        console2.log("Gas Limit:", config.gasLimit);

        vm.startBroadcast(deployerPrivateKey);

        // Deploy the AutomatedSolarpark contract
        AutomatedSolarpark automatedSolarpark = new AutomatedSolarpark(config.router);

        console2.log("AutomatedSolarpark deployed at:", address(automatedSolarpark));
        console2.log("Owner:", automatedSolarpark.owner());

        // Configure the contract with optimistic CBOR request
        console2.log("Configuring contract with optimistic CBOR request...");
        
        automatedSolarpark.updateRequest(
            SIMPLE_FETCH_REQUEST, // Pre-encoded simple fetch request
            config.subscriptionId,
            config.gasLimit,
            config.donId
        );
        
        console2.log("Contract configured successfully with simple fetch request");

        vm.stopBroadcast();

        console2.log("\nDeployment Summary:");
        console2.log("==================");
        console2.log("Contract Address:", address(automatedSolarpark));
        console2.log("Router:", config.router);
        console2.log("DON ID:", vm.toString(abi.encodePacked(config.donId)));
        console2.log("Installation ID:", automatedSolarpark.getInstallationId());
        
        console2.log("\nAutomated Solar Park Configuration:");
        console2.log("==================================");
        console2.log("CBOR Request: Pre-configured with corrected simple-fetch-readings.js");
        console2.log("API Workflow: GET /api/v1/readings/pending -> Return data for on-chain storage");
        console2.log("Subscription ID:", config.subscriptionId);
        console2.log("Gas Limit:", config.gasLimit);
        console2.log("Automation Ready: YES - Contract supports Chainlink Automation");
        
        console2.log("\nNext Steps:");
        console2.log("===========");
        console2.log("1. Add this contract as a consumer to subscription", config.subscriptionId);
        console2.log("2. Set up Chainlink Automation upkeep for automated calls");
        console2.log("3. Call setAutomationCronContract() with your upkeep contract address");
        console2.log("4. Test manual execution by calling sendRequestCBOR() as owner");
        console2.log("5. Monitor Response events for successful automated runs");
        
        console2.log("\nAutomation Setup Commands:");
        console2.log("=========================");
        console2.log("Set automation contract:");
        console2.log("cast send", address(automatedSolarpark), "'setAutomationCronContract(address)' <UPKEEP_ADDRESS> --private-key $DEPLOYER_PRIVATE_KEY --rpc-url $AVALANCHE_FUJI_RPC");
        console2.log("\nManual test:");
        console2.log("cast send", address(automatedSolarpark), "'sendRequestCBOR()' --private-key $DEPLOYER_PRIVATE_KEY --rpc-url $AVALANCHE_FUJI_RPC");
        console2.log("\nCheck readings count:");
        console2.log("cast call", address(automatedSolarpark), "'getReadingsCount()' --rpc-url $AVALANCHE_FUJI_RPC");
    }
} 